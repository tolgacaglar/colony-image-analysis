...
tar = np.linspace(0, 23*60 + 9 + 2.87/60, 27)
# tlen = 27
zlen = 138
tlen = 1
# zlen = 1
...
test_file =  folder + "/Raw_Images/ColonyGrowth_EQ59_11172020_Mark_and_Find_002_Position5005_t00_z000_ch00.tif"
img_test = cv2.imread(test_file)
height,width,layers = img_test.shape
size = (width,height)

out = cv2.VideoWriter(folder + "/test_height.avi", cv2.VideoWriter_fourcc(*'MP42'), float(4), size)
for tix in range(tlen):
    img_zsum = np.zeros(zlen)
    for zix in range(zlen):
        t_str = "t%02d" % (tix+15)
        z_str = "z%03d" % (zix)
        filename = "C:/Users/Tolga/Dropbox/Raw Data/Colony Images - Confocal/Colony Images/Raw_Images/ColonyGrowth_EQ59_11172020_Mark_and_Find_002_Position5005_" + t_str + "_" + z_str + "_ch00.tif"
